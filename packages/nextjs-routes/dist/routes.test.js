...
describe('routes()', () => {
    describe('정적 경로', () => {
        it('/ 루트 경로를 반환한다', () => {
            expect(routes({ pathname: '/' })).toBe('/');
        });
        it('정적 pathname을 그대로 반환한다', () => {
            expect(routes({ pathname: '/404' })).toBe('/404');
        });
        it('중첩 정적 경로를 반환한다', () => {
            expect(routes({ pathname: '/blog/about' })).toBe('/blog/about');
        });
    });
    describe('동적 경로', () => {
        it('[slug] 파라미터를 치환한다', () => {
            expect(routes({ pathname: '/blog/[slug]', params: { slug: 'hello' } })).toBe('/blog/hello');
        });
        it('다중 동적 파라미터를 치환한다', () => {
            expect(routes({
    ...
    describe('search params', () => {
        it('단일 search 파라미터를 추가한다', () => {
            expect(routes({ pathname: '/blog', search: { page: '1' } })).toBe('/blog?page=1');
        });
        it('복수 search 파라미터를 추가한다', () => {
            const result = routes({ pathname: '/blog', search: { page: '1', sort: 'desc' } });
            expect(result).toBe('/blog?page=1&sort=desc');
        });
        it('undefined search 값은 필터링한다', () => {
            expect(routes({ pathname: '/blog', search: { page: undefined } })).toBe('/blog');
        });
        it('빈 search 객체는 쿼리를 추가하지 않는다', () => {
            expect(routes({ pathname: '/blog', search: {} })).toBe('/blog');
        });
    });
    describe('hash', () => {
        it('hash를 추가한다', () => {
            expect(routes({ pathname: '/blog', hash: 'section-1' })).toBe('/blog#section-1');
        });
    });
    describe('복합', () => {
