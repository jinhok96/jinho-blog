...
describe('findFiles', () => {
    ...
    it('파일 목록을 반환한다', () => {
        mockReaddirSync.mockReturnValue(['page.tsx', 'layout.tsx']);
    ...
    it('디렉토리를 재귀적으로 탐색한다', () => {
        mockReaddirSync.mockImplementation((dir) => {
            if (dir === '/root')
                return ['blog'];
            if (dir === join('/root', 'blog'))
                return ['page.tsx'];
            return [];
        });
        mockStatSync.mockImplementation((p) => ({ isDirectory: () => p === join('/root', 'blog') }));
        const result = findFiles('/root');
        expect(result).toEqual([join('/root', 'blog', 'page.tsx')]);
    });
    it('node_modules 경로를 제외한다', () => {
        mockReaddirSync.mockReturnValue(['node_modules', 'page.tsx']);
        mockStatSync.mockImplementation((p) => ({ isDirectory: () => String(p).includes('node_modules') }));
        const result = findFiles('/root');
