...
async function cli(): Promise<void> {
  ...
  const hasPagesDir =
    existsSync(join(dir, "pages")) || existsSync(join(dir, "src", "pages"));

  if (!hasAppDir && !hasPagesDir) {
    console.error(
      "❌ Error: Could not find 'app' or 'pages' directory in the current directory."
    );
    console.error(
      `   Expected to find either in: ${dir}\n`
    );
    console.error(
      "   Make sure you're running this command in a Next.js project root."
    );
    process.exit(1);
  }
